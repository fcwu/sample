...
class MyLogger(logging.Logger):
    ...
    def __init__(self, name):
        ...
        color_formatter = ColoredFormatter(MyLogger.COLOR_FORMAT)
        no_color_formatter = ColoredFormatter(MyLogger.NO_COLOR_FORMAT,
                                              False)

        #create the single file appending handler
        if MyLogger.LOG_FILE_HANDLER == None:
            filename = os.path.join('/tmp', 'your.log')
            MyLogger.LOG_FILE_HANDLER = logging.FileHandler(filename, 'w')
            MyLogger.LOG_FILE_HANDLER.setFormatter(no_color_formatter)

        console = logging.StreamHandler()
        console.setFormatter(color_formatter)

        self.addHandler(MyLogger.LOG_FILE_HANDLER)
